...
thai_effort_processed %>% 
  ...
  xlab("Predict annual hours fished/n[hrs/year]") +
  ylab("Actual annual hours fished/n[hrs/year]")
...
if (requery == "yes"){
  ...
  fao_file <- paste("all_fishing_effort_all_years")
  
  vesselQueryGlobal <- tbl(GFW_connection, from = fao_file) %>%
    dplyr::select(timestamp, year, mmsi, flag_country_name, hours, measure_new_score, distance_from_shore, implied_speed) %>%
    collect(n=Inf) %>%
    rename(Year = year) %>%
    mutate(Date = date(timestamp)) %>%
    mutate(Fishing = ifelse(measure_new_score >= 0.5 & 
                              !((distance_from_shore < 1000 & implied_speed < 1)),1,0)) %>%
    group_by(mmsi, Year, flag_country_name) %>%
    summarize(total_hours_fished = sum(hours*Fishing),
              Annual_Active_Days = n_distinct(Date),
              Annual_Days_Fishing = n_distinct(Date[Fishing==1]),
              number_days_transit = n_distinct(Date[Fishing==0]),
...
global_effort_processed <- vesselQueryGlobal %>%
  left_join(vessel_characteristics,by="mmsi")
