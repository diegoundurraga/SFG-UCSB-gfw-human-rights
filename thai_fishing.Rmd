...
thai_effort_processed <- vesselQueryAll %>%
  ...
  summarize(total_hours_fished = sum(hours*Fishing),
            number_days_pinged = n_distinct(Date),
            ...
            day_ratio = number_days_fishing/number_days_pinged) %>%
  left_join(vessel_characteristics,by="mmsi") %>%
    arrange(-number_days_fishing)

thai_effort_processed %>%
  plot_ly(x=~number_days_fishing,type="histogram",text=~mmsi) %>%
  layout(barmode = "overlay")

modelFit <- lm(number_days_fishing~number_days_transit+0,thai_effort_processed,offset=rep(0, nrow(thai_effort_processed)))
slope <- tidy(modelFit)$estimate
thai_effort_processed %>%
  plot_ly(x= ~number_days_transit, y =~number_days_fishing,
          mode="scatter") %>%
  add_markers(size=~number_days_pinged,
              color=~number_days_pinged,
              text = ~paste('Vessel: ', mmsi,
                            '<br>Year: ',Year,
                            '<br>Annual Hours Fished: ',round(total_hours_fished),
                            '<br>Annual Days Fishing: ',round(number_days_fishing),
                            '<br>Annual Days Pinged: ',round(number_days_pinged))) %>%
  add_trace(x = c(0, max(thai_effort_processed$number_days_transit)), 
            y = c(0, slope*max(thai_effort_processed$number_days_transit)),mode="lines")
```
...
leaflet() %>%
  ...
  addLayersControl(
    overlayGroups = c("Fishing", "Transit","Port"),
