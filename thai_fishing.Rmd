...
```{r}
HRModel <- glm(Offender ~ hours_per_day + Annual_Days_Fishing, data = thai_effort_processed, family = "binomial")
...
thai_effort_processed$offenderLikelihood <- offenderLikelihood
	
markerColors = list(color = colorRampPalette(brewer.pal(11,"Spectral"))(100))

thai_effort_processed %>%
  plot_ly(x= ~Annual_Active_Days, y =~hours_per_day,
          mode="scatter") %>%
  add_markers(size=~offenderLikelihood,
              color=~offenderLikelihood,
              colors=c("black","red"),
              text = ~paste('Vessel: ', mmsi,
                            '<br>Year: ',Year,
                            '<br>Annual hours fished: ',round(total_hours_fished),
