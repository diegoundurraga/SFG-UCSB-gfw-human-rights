...
vesselQueryGlobalProcessed <- vesselQueryGlobal %>%
  ...
  mutate(Annual_Active_Days = pmax(Annual_Active_Days,Annual_Days_Fishing),
         ...
         day_ratio = Annual_Days_Fishing/Annual_Active_Days) %>%
  select(-`0_hours`)  %>%
  left_join(vesselInfo[!duplicated(vesselInfo$mmsi),],by="mmsi") %>%
...
vesselQueryGlobalProcessed %>% 
  mutate(Offender = ifelse(mmsi %in% c(567000445,567000421,567025800) & year == 2015,"Offender","Non_Offender")) %>%
  arrange(Offender) %>%
  ggplot(aes(x=pred,y=hours_per_day)) +
  geom_point(aes(color=Offender,alpha=Offender)) +
  geom_abline(intercept = 0, slope = 1,color="blue",alpha=0.5) +
  xlab("Predicted hours fished per day\n[hours/day]") +
  ylab("Actual hours fished per day\n[hours/day]") + 
  scale_alpha_manual(guide='none', values = list(Non_Offender = 0.2, Offender = 1)) + 
  scale_color_manual(guide='none', values = c("gray","red"))
```
