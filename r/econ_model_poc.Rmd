...
risky_vessels <- results %>% 
  ...
  mutate(on_offender_list = ifelse(mmsi %in% thai_mmsi,TRUE,FALSE)) %>%
  arrange(desc(max_risk_score))

bq_table(project = project,table = "poc_risky_vessels",dataset = "human_rights") %>% 
  bq_table_upload(values = risky_vessels)
...
`human_rights.poc_voyages_gridded`
"
results_gridded <- bq_project_query(project, sql) %>%
  bq_table_download(max_results = Inf)
```
